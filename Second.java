...
public class Second extends JFrame {
	...
	public Second() {
		...
		btnSearchNearestMetro.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					Connection con = (Connection) DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe",
							"system", "SYSTEM#2429");
					String query = "SELECT * FROM locationn where Prime_Location=?";
					...
					while (rs.next()) {
						intFlag = 1;
						float distt = rs.getFloat("Distance");
						String value = rs.getString("Nearest_Metro");
		...
		JButton btnViewPrimeLocations = new JButton("VIEW PRIME LOCATIONS PRESENT IN OUR DATABASE");
		btnViewPrimeLocations.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					...
					Connection con = (Connection) DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe",
							"system", "SYSTEM#2429");
					String query = "SELECT UNIQUE Prime_Location FROM locationn";
					pst = (OraclePreparedStatement) con.prepareStatement(query);
					ResultSet rs = (OracleResultSet) pst.executeQuery();

					DefaultListModel<String> model = new DefaultListModel<String>();
					while (rs.next()) {
						String pl = rs.getString("Prime_Location");
						list_1.setModel(model);
						model.addElement(pl);
					}
					JList<String> list = new JList<String>(model);
		...
		btnViewPrimeLocations.setBounds(526, 29, 348, 48);
		contentPane.add(btnViewPrimeLocations);
		}
}
